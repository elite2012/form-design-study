...
const getOperateStyle = (schema: any) => {
  let style: any = {};
  const widgetName =
    schema?.items?.theme || schema?.items?.widget || "collapse";
  if (
    ["card", "collapse", "lineTitle"].includes(widgetName) &&
    !schema?.items?.props?.extra
  ) {
    style.top = "14px";
    ...
    <>
      ...
      >
        {fields.map(({ key, name }) => {
          ...
          if (
            newSchema.items.title &&
            typeof newSchema.items.title === "string"
          ) {
